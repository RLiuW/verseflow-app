...
class AdminRegisterCard extends StatefulWidget {
  ...
  const AdminRegisterCard(this.snapshot, {super.key});

  @override
  _AdminRegisterCardState createState() => _AdminRegisterCardState();
}
...
class _AdminRegisterCardState extends State<AdminRegisterCard> {
                    ...
                    children: [
                      TextButton(
                          ...
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: <Widget>[
                              Text(l10n.deleteRegistration, style: TextStyle(color: scheme.onSurfaceVariant),),
                      ...
                      FilledButton.icon(
                          ...
                          label: Text(l10n.saveRegistration),
                      )
                    ],
                  )
                ],
    ...
    showDialog(
      ...
      builder: (dialogContext) {
        ...
        return CupertinoAlertDialog(
          title: Center(
            child: Text(
              l10nDialog.confirmDeleteRegistration,
            ...
            CupertinoDialogAction(
              child: GestureDetector(
                onTap: () async {
                  ...
                  await _data.collection("cadastros").doc(docID).delete();
                  if (!dialogContext.mounted) return;
                  Navigator.of(dialogContext).pop();
                  if (context.mounted) _showToast(l10n.registrationRejected);
                },
  ...
  Future<void> _saveData() async {
    final userId = widget.snapshot.id;
    final l10n = AppLocalizations.of(context);
    showDialog(
      context: context,
      builder: (dialogContext) {
          ...
          actions: [
            ...
            CupertinoDialogAction(
              child: GestureDetector(
                onTap: () async {
                  try {
                    await _data.collection("cadastros").doc(userId).update({
                      "status": "aprovado",
                      "approvedBy": AppConstants.adminEmail,
                      "approvedAt": Timestamp.now(),
                    });
                    final snapshot = await _data.collection("cadastros").doc(userId).get();
                    final docData = snapshot.data() ?? {};
                    await _data.collection("usuarios").doc(userId).set({
                      "nome": docData["nome"] ?? "",
                      "email": docData["email"] ?? "",
                      "data": docData["data"],
                      "approvedAt": Timestamp.now(),
                    });
                    await _data.collection("cadastros").doc(userId).delete();
